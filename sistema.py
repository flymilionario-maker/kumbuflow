import os  # CORRIGIDO: Faltava este import essencial para o Render funcionar!
import sqlite3
import psycopg2
...
# CORRIGIDO: Configuração para o login NÃO desaparecer após 1 hora
app.permanent_session_lifetime = datetime.timedelta(days=7)

@app.before_request
def manter_sessao_permanente():
...
def verificar_acesso():
    # CORRIGIDO: Evita que erros internos travem o redirecionamento com BuildError
    if not request.endpoint:
        return None
    if request.endpoint in ['login', 'static', 'debug_usuarios']:
        return None
...
def dashboard():
    ...
    # CORRIGIDO: Lógica inteligente que escolhe a query certa dependendo do Banco de Dados ativo
    if os.environ.get('DATABASE_URL'):
        # Query para Postgres (Render)
        faturacao_mensal = query_db('''SELECT SUM(total) as faturacao_mensal FROM vendas 
                                       WHERE usuario_id = ? AND TO_CHAR(data_venda, 'YYYY-MM') = TO_CHAR(CURRENT_DATE, 'YYYY-MM')''', (usuario_logado,), one=True)
    else:
        # Query para SQLite (Local)
        faturacao_mensal = query_db('''SELECT SUM(total) as faturacao_mensal FROM vendas 
